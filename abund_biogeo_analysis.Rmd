...
```{r}
getwd()
knitr::opts_knit$set(root.dir = "/home/zavadska/Documents/biology/BSD3-main/")
```
...
"In variables presented as proportions or ratios, e.g., humidity, this problem can be overcome with the arc-sin transformation. In those variables stretched over a large scale of values, e.g., illumination and passage cross section, this
can be achieved by transformation in the logarithmic scale."
we will use log-transformation to normalize all out data; we also need to replace 0s
```{r}
#log(for_rda[,10:ncol(for_rda)])
min(for_rda[,10:ncol(for_rda)], na.rm = TRUE)
